"""Expectation maximization hierarchical DPMM with diagonal covariance for OpenOOD
...
# Logging
parser.add_argument("--log_path", type=str, default="em_diag_dpmm.log")
parser.add_argument("--output_filename", type=str, default="em_diag_dpmm.result")
# Model
...
def main(args):
    ...
    logger.info("EM Fitting Diagonal Cov Hierarchical DPMM on ViT Features for OpenOOD")
    logger.info("---------------------------------------------------------------------")
    ...
    lls, kappas, nus = dhdpmm.em(Nk, sumx, sumxsq,
                                 init_nu0, sigmasq0_star, init_kappa0, mu0_star,
                                 num_iter=100)
    ...
    posterior_params = dhdpmm.compute_posterior_params(
        Nk, sumx, sumxsq, torch.as_tensor(nu0_star, device=DEVICE),
    ...
    def compute_scores(X, prior_params, posterior_params, return_preds=False):
        return dhdpmm.compute_scores(prior_params, posterior_params, X,
                                     return_preds=return_preds,
                                     Ns=Nk,
                                    )
    aurocs = openood_eval(dataset_dict, params, compute_scores, model="Diag DPMM")
    output["openood"] = aurocs
    logger.info("EM Diagonal HDPMM AUROC:")
