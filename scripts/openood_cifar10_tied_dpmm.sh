...
for iter in 0 1 2; do
    CUDA_VISIBLE_DEVICES=$cuda_device_id python train_tied_dpmm_openood.py \
        ...
        --model-iter $iter \
        --log_path cifar10_openood_exps/tied/logs/${featprefix}s${iter}.log \
        --output_filename cifar10_openood_exps/tied/results/${featprefix}s${iter}.result
    ...
    CUDA_VISIBLE_DEVICES=$cuda_device_id python train_tied_dpmm_openood.py \
        ...
        --data-root cifar10_feats/ \
        --model-iter $iter \
        --log_path cifar10_openood_exps/tied/logs/${featprefix}s${iter}_autowhiten.log \
        --output_filename cifar10_openood_exps/tied/results/${featprefix}s${iter}_autowhiten.result
done
