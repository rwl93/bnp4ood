...
for iter in 0 1 2; do
    CUDA_VISIBLE_DEVICES=$cuda_device_id python train_em_coupled_diag_hdpmm_openood.py \
        ...
        --model-iter $iter \
        --log_path cifar100_openood_exps/coupled_diag/logs/${featprefix}s${iter}.log \
        --output_filename cifar100_openood_exps/coupled_diag/results/${featprefix}s${iter}.result
    ...
    CUDA_VISIBLE_DEVICES=$cuda_device_id python train_em_coupled_diag_hdpmm_openood.py \
        ...
        --data-root cifar100_feats/ \
        --model-iter $iter \
        --log_path cifar100_openood_exps/coupled_diag/logs/${featprefix}s${iter}_autowhiten.log \
        --output_filename cifar100_openood_exps/coupled_diag/results/${featprefix}s${iter}_autowhiten.result
done
